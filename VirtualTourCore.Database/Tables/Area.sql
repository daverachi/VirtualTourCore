CREATE TABLE [dbo].[Area]
(
	Id INT IDENTITY (1,1) NOT NULL,
	ClientId INT NOT NULL,
	LocationId INT NOT NULL,
	CustomizationId INT NULL,
	ItemStatusId INT NULL,
	Name NVARCHAR(50) NOT NULL,
	Description VARCHAR (MAX) NULL,
    DescriptionHtml VARCHAR (MAX) NULL,
    DescriptionJson VARCHAR (MAX) NULL,
	AssetAreaId		INT NULL,
	CreateUserID    INT NOT NULL,
    CreateDate      DATETIME      NOT NULL,
    UpdateUserID    INT			NULL,
    UpdateDate      DATETIME      NULL, 
	CONSTRAINT PK_Area PRIMARY KEY CLUSTERED (Id ASC),
    CONSTRAINT FK_Area_ItemStatus FOREIGN KEY (ItemStatusID) REFERENCES dbo.ItemStatus (Id),
    CONSTRAINT FK_Area_Customization FOREIGN KEY (CustomizationId) REFERENCES dbo.Customization (Id),
    CONSTRAINT FK_Area_AssetImg FOREIGN KEY ([AssetAreaID]) REFERENCES dbo.AssetStore (Id),
	CONSTRAINT FK_Area_Client FOREIGN KEY (ClientId) REFERENCES dbo.Client (Id),
	CONSTRAINT FK_Area_Location FOREIGN KEY (LocationId) REFERENCES dbo.Location (Id),
	CONSTRAINT FK_Area_CreateUser FOREIGN KEY (CreateUserId) REFERENCES dbo.SecurityUser (Id),
    CONSTRAINT FK_Area_UpdateUser FOREIGN KEY (UpdateUserId) REFERENCES dbo.SecurityUser (Id),
)
